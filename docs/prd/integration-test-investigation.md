...
> **Rev 6(2026-09-21)修订要点**
>
> - SKILL.md 按四层结构重写:骨架=流程图(编号步骤+早退门)、路由=条件表(可观察条件→路径,最便宜决定性信号优先)、方法=自由区(每条路径只固定必答问题与出口条件)、终局=判断点(强制产出物);写作标准(流程体):文档按可遍历流程组织,句子单元只允许步骤、规则(IF-THEN)或 gotcha;默认规则、自由仅在显式判断点授予(§8、§15)
>
...
- `SKILL.md`:frontmatter 触发描述 + 四层结构协议(流程图骨架、条件表路由、自由区路径、判断点终局,见 §15)+ 跨阶段硬规则;以**流程体**写成——文档按可遍历的流程组织,句子层面的单元只允许三种:步骤、规则(IF-THEN / 条件→动作)、gotcha,无法归入任何单元的句子删除(体裁 > 骨架 > 句子单元三层各司其职,防止"流程体"被误读为全面步骤化)
...
## 15. Investigation 结构:固定骨架,自由方法

调查协议按四层结构实现,每层文体不同,不混写:

```text
骨架 = 流程图   阶段顺序、早退门、入口 / 出口条件
路由 = 条件表   IF 可观察条件 THEN 调查路径;默认"最便宜的决定性信号优先"
方法 = 自由区   每条路径只固定"必须回答的问题"与"出口条件",探索方法不规定
终局 = 判断点   充分性与证据权衡是唯一合法的判断区,以强制产出物约束
```

固定与自由的边界:固定"问什么、何时停",放开"怎么找"。

- 默认是规则,自由只以显式判断点的形式授予("默认规则 + 显式授权的自由"),防止退化成议论散文
- Agent 不机械遍历所有路径:路由表只触发命中条件的路径;某 Cluster 的成因已有直接证据时,不再遍历其余路径
- 早退门只接受直接证据,不接受"看起来像"——不允许在第一个可疑信号后停止
